...
// the live CRM integration keeps working exactly as before — only where
// the function reads the expected token from changed.
...
Deno.serve(async (req) => {
  ...
  const { phone, briefing_id, score, completed_at, answers } = body;
  ...
  completions[agentId] = {
    name: agent.name,
    score,
    signed_at: completed_at || new Date().toISOString(),
    answers: answers || [],
    source: 'crm'
  };
